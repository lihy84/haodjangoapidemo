...
def table_metadata(request):
    ...
    if not db_string:
        return HttpResponseBadRequest(content='{"error": "No database connection string provided."}', content_type=content_type)

    # If no table name was provided, return a 400 (Bad Request) response
    if not table_name:
        return HttpResponseBadRequest(content='{"error": "No table name provided."}', content_type=content_type)
    ...
    try:
        ...
        return HttpResponseBadRequest(content='{"error": "Could not connect to database. Error: ' + str(e) + '"}', content_type=content_type)

    # If the table does not exist in the database, return a 400 (Bad Request) response
    if table_name not in inspector.get_table_names():
        return HttpResponseBadRequest(content='{"error": "Table does not exist in the database."}', content_type=content_type)
    ...
    try:
        ...
        return HttpResponseBadRequest(content='{"error": "Could not retrieve table metadata. Error: ' + str(e) + '"}', content_type=content_type)
